...
| 14 | camera_desk | 오생성 | 책상 잡동사니는 사라졌으나, 원본의 작은 공 모양 카메라가 완전히 다른 모양의 대형 렌즈로 왜곡 생성되고 받침대 하단에 없던 질감이 추가됨(확대 전엔 안 보임) |
...
확대 재검토를 통과한 유일한 사례. 상자 안 과일 9개(사과·배·천도복숭아 등)의 개수·모양·배치가
...
원본 구석에 작게 걸쳐 있던 공 모양 카메라가, 완전히 다른 형태의 대형 카메라 렌즈로 왜곡 생성됐다.
...
3. **"오생성" 패턴 (16건)**: 압도적인 실패 원인이고, 네 가지 하위 유형 + 워터마크 재현형으로 나뉜다.
   ...
   - **경계 사물 복제/왜곡형 (4건: 06, 07, 08, 14번)**: 마스크 경계에 걸쳐 있던 기존 사물(탁상시계,
     병, 효자손, 카메라)이 diffusion에 의해 복제되거나 다른 모습으로 왜곡되는 경우. **가장 위험한
