...
**집계**: 성공 **8/20 (40%)**, 무변화 3/20 (15%), 오생성 9/20 (45%)

이전 반복(YOLOv8-seg 기반, 성공 7/20·35%) 대비 성공률이 소폭 올랐다. rembg가 20장 전부에서
GrabCut 폴백 없이 안정적으로 마스크를 뽑아준 덕에 "상품 인식 실패"로 인한 실패가 사라졌고, 남은
실패는 전부 diffusion 자체의 문제(오생성·워터마크 재현·경계 사물 복제)다.
...
처음엔 "타일 벽과 탁상시계가 사라지고 회색 텍스처 배경만 남은 성공 사례"로 분류했었다. 그런데
확대해보니 시계는 사라진 게 아니라 **2~3개로 겹쳐 복제**돼 있었다 — 원래 하나뿐인 탁상시계가 마스크
경계에 걸쳐 있다 보니, diffusion이 그 형태를 배경 소재로 착각해 반복 생성한 것으로 보인다. 효자손
사례와 똑같이 "마스크 경계에 걸친 사물이 복제되는" 패턴이라, 재분류했다.
...
**공통 발견**: 오생성은 크게 두 갈래였다. 하나는 diffusion이 넓거나 색이 짙은 배경 영역을 "clean
studio background" 프롬프트에 맞춰 과도하게 해석해 원본에 없던 소품·장식을 아예 새로 그려 넣는
경우(03, 04, 05, 13, 18, 19번). 다른 하나는 backscratcher(08번)와 swabbox_bathroom(06번)처럼
**마스크 경계에 걸쳐 있던 기존 사물을 diffusion이 반복·복제**하는 경우다. 둘 다 마스크 품질과 무관하게
diffusion 모델 자체의 특성이라, rembg로 마스크를 개선해도 해결되지 않았다. 특히 후자는 "성공"으로 보고
넘어가기 쉽다는 게 문제인데 — 사물이 사라진 배경처럼 보여서 마스크가 잘 뽑힌 것으로 착각하기 쉽지만,
실제로는 경계에 걸친 사물이 남아있는 정도가 아니라 늘어나 있다.
...
**미달이지만 개선됨.** 40%(8/20)로, 1차 PoC(35%)보다는 나아졌지만 목표 80%에는 여전히 못 미친다.
...
무변화/오생성 케이스(12/20)는 여전히 수작업을 대체하지 못해 검수 시간이 추가로 필요하다 — 그 시간까지
포함하면 실질적인 단축 폭은 위 숫자보다 작다.

### ③ 합성 결과가 자연스러워 바로 쓸 수 있는 비율
①과 동일하게 40% 수준. 나머지 절반이 조금 넘는 비율은 원본을 그대로 쓰거나(무변화) 재작업이 필요하다(오생성).
...
3. **"오생성" 패턴 (9건)**: 여전히 가장 큰 실패 원인이고, 세 가지 하위 유형으로 나뉜다.
   - **소품·장식 창작형 (6건: 03, 04, 05, 13, 18, 19번)**: `runwayml/stable-diffusion-inpainting`
     (SD 1.5 기반)이 "clean seamless white studio background" 프롬프트를 과도하게 해석해 조명 장비,
     장식품 등 원본에 없던 사물을 새로 만들어내는 경우.
   - **경계 사물 복제형 (2건: 06, 08번)**: 마스크 경계에 걸쳐 있던 탁상시계(06번)나 상품 자체인
     효자손(08번)이 diffusion에 의해 2~3개로 반복 생성되는 경우. 육안으로는 "사물이 사라진 깔끔한
     배경"처럼 보이기 쉬워서, 확대해서 개수를 세어보기 전까진 성공으로 오분류하기 쉬운 유형이다(실제로
     06번을 처음엔 성공으로 분류했었다).
   - **스톡 워터마크 재현형 (1건: 12번)**: 확대해보니 배경 구석에 스톡 사진 워터마크(로고 + ID 번호
     패턴)가 생성돼 있었다. 이건 모델이 "그럴듯한 소품"을 창작한 게 아니라, 학습 데이터(웹 크롤링
     데이터셋에 섞인 워터마크 있는 스톡 이미지)에서 본 패턴을 기억해서 재현한 것에 가깝다 — diffusion
     모델에 알려진 memorization(암기) 현상 중 하나다.

   세 유형 다 마스크를 rembg로 바꾼 뒤에도 그대로 남아있어, 이 문제는 분할 모델이 아니라 diffusion
   모델/프롬프트 자체의 한계라는 결론은 동일하다.
4. **성공 사례의 공통점 (8건)**: 상품 윤곽이 뚜렷하고 배경이 밝거나 중간 톤인 경우 결과가 안정적이었다.
   반대로 배경이 어둡거나(무드조명, 창밖 야경) 색이 진할수록 diffusion이 환각을 일으키는 빈도가 높았다.
   다만 "경계 사물 복제형"이 한 건 더 숨어있다가 뒤늦게 발견된 걸 감안하면, 나머지 8건도 확대해서
   재검토하지 않는 한 안심할 수 없다.
...
- 12번에서 발견된 워터마크 재현은 별도 원인(memorization)이라, `watermark, text, logo, signature,
  stock photo` 같은 워터마크 특화 네거티브 프롬프트로 따로 검증 필요 — 일반 오생성 억제 프롬프트로
  해결되는지는 아직 확인 안 됨
- "성공" 8건도 전부 확대해서 재검토 필요 — 06번(swabbox_bathroom)이 육안으로는 성공처럼 보였지만
  실제로는 경계에 걸친 탁상시계가 복제돼 있었다. 마스크 경계에 걸친 배경 사물이 있는 이미지는 특히
  주의해서 봐야 한다.
