...
**집계**: 성공 **9/20 (45%)**, 무변화 3/20 (15%), 오생성 8/20 (40%)

이전 반복(YOLOv8-seg 기반, 성공 7/20·35%) 대비 성공률이 올랐다. rembg가 20장 전부에서
GrabCut 폴백 없이 안정적으로 마스크를 뽑아준 덕에 "상품 인식 실패"로 인한 실패가 사라졌고, 남은
실패는 전부 diffusion 자체의 문제(오생성·워터마크 재현)다.
...
**미달이지만 개선됨.** 45%(9/20)로, 1차 PoC(35%)보다는 나아졌지만 목표 80%에는 여전히 못 미친다.
...
무변화/오생성 케이스(11/20)는 여전히 수작업을 대체하지 못해 검수 시간이 추가로 필요하다 — 그 시간까지
포함하면 실질적인 단축 폭은 위 숫자보다 작다.

### ③ 합성 결과가 자연스러워 바로 쓸 수 있는 비율
①과 동일하게 45% 수준. 나머지 절반이 조금 넘는 비율은 원본을 그대로 쓰거나(무변화) 재작업이 필요하다(오생성).
...
3. **"오생성" 패턴 (8건)**: 여전히 가장 큰 실패 원인. 대부분은 `runwayml/stable-diffusion-inpainting`
   (SD 1.5 기반)이 "clean seamless white studio background" 프롬프트를 과도하게 해석해 조명 장비,
   장식품, 심지어 상품 자체의 복제본까지 만들어내는 경우였다. 다만 12번은 조금 다른 유형이다 — 확대해보니
   배경 구석에 스톡 사진 워터마크(로고 + ID 번호 패턴)가 생성돼 있었다. 이건 모델이 "그럴듯한 소품"을
   창작한 게 아니라, 학습 데이터(웹 크롤링 데이터셋에 섞인 워터마크 있는 스톡 이미지)에서 본 패턴을
   기억해서 재현한 것에 가깝다 — diffusion 모델에 알려진 memorization(암기) 현상 중 하나다. 두 유형 다
   마스크를 rembg로 바꾼 뒤에도 그대로 남아있어, 이 문제는 분할 모델이 아니라 diffusion 모델/프롬프트
   자체의 한계라는 결론은 동일하다.
4. **성공 사례의 공통점 (9건)**: 상품 윤곽이 뚜렷하고 배경이 밝거나 중간 톤인 경우 결과가 안정적이었다.
